-- adjustSoXEffectNames -- changes descriptions of SoX effects
--                         in effects lists
--
--
-- by Dr. TT, 2023
...
local _programName = "adjustSoXEffectNames"
local _configurationFileName = "LTBVC_" .. _programName .. ".cfg"
...
function _adjustEffectsInTracks (trackList)
...
function _simpleEffectKind (extendedKind)
    ...
    if String.find(extendedKind, ".vst")
        or String.find(extendedKind, ".dll") then
        -- parse the string for the effect kind
        local pattern = "(%a+).[vd][sl][lt]"
        local captureList = String.captureList(extendedKind, pattern)

        if captureList ~= nil then
            result = captureList:at(1)
        end
    end
...
function main ()
    ...
    local effectNameString = _adjustEffectsInTracks(trackList)
